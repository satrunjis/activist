...
$container = "activist-base-pg"
$dbUser = "postgres"
$dbName = "activist_base"
...
docker exec -i $container psql -U $dbUser -d $dbName -v ON_ERROR_STOP=1 -c $sql | Out-Host

Write-Host "[dev-seed] seeded users:"
Write-Host "  login=admin password=admin"
Write-Host "  login=test0 password=test0"
Write-Host "  login=seed_target password=test0"
Write-Host "[dev-seed] ensured root division id=seed-root"
Write-Host "[dev-seed] ensured additional demo divisions for tree preview"
Write-Host "[dev-seed] ensured role fixtures via direct DB write (standard/manager/leader/deputy)"
Write-Host "[dev-seed] ensured phase-4 acceptance fixtures (seed-pos-manager, seed-pos-leader, seed-pos-field)"
Write-Host "[dev-seed] assigned admin to system-admin role at root division"
Write-Host "[dev-seed] ensured seed-role-manager user account (login=role_manager password=test0) - role/position/membership prepared in DB seed"
