...
import type { EditRoleRequest, PermissionScope, RolePermission, RoleWithPermissions } from "../../shared/api/types";
...
function buildInitialScopeMap(permissions: RolePermission[] | undefined): Record<string, PermissionScope> {
  const base = PERMISSION_OPTIONS.reduce<Record<string, PermissionScope>>((acc, option) => {
    acc[option.value] = option.defaultScope;
    return acc;
  }, {});
  for (const permission of permissions ?? []) {
    base[permission.code] = permission.scope;
  }
  return base;
}

function toPayload(values: RoleFormValues, scopeByCode: Record<string, PermissionScope>): EditRoleRequest {
...
export function RoleEditForm({ role, onSuccess, onCancel }: RoleEditFormProps) {
  ...
  const [scopeByCode, setScopeByCode] = useState<Record<string, PermissionScope>>(() =>
    buildInitialScopeMap(role.permissions)
  );
  ...
  useEffect(() => {
    ...
    setScopeByCode(buildInitialScopeMap(role.permissions));
  }, [form, role]);
  ...
  async function handleSubmit(values: RoleFormValues) {
    ...
    try {
      await request(`/api/v1/roles/${encodeURIComponent(role.id)}`, { method: "PATCH", body: toPayload(values, scopeByCode) });
      form.reset({
        name: values.name.trim(),
      ...
      const apiError = adaptApiError(error, "Не удалось сохранить роль.");
