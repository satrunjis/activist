...
type Service interface {
	SearchUsers(ctx context.Context, actorID shared.UserID, input appsearch.SearchUsersInput) (appsearch.SearchUsersResult, error)
}
...
func RegisterRoutes(r chi.Router, handler *Handler, authMiddleware Authenticator) {
	...
	r.Route("/search", func(searchRouter chi.Router) {
		if authMiddleware != nil {
			searchRouter.Use(authMiddleware.Authenticate)
		}
		searchRouter.Get("/users", handler.GetUsers)
	})
}
...
func (h *Handler) GetUsers(w http.ResponseWriter, r *http.Request) {
	...
	request, err := parseUsersSearchRequest(r.URL.Query())
	if err != nil {
		writeError(w, errEnvelopeFrom(err))
		return
	}
	result, err := h.service.SearchUsers(r.Context(), actor.UserID, request.toInput())
	if err != nil {
		writeError(w, errEnvelopeFrom(err))
		return
	}
	writeJSON(w, http.StatusOK, newUsersSearchResponse(result))
}
